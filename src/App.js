...
function App() {
  ...
  return (
    ...
    <section className="w-screen h-screen bg-slate-900 flex justify-evenly flex-wrap items-center border-b border-slate-400">
      <div className="todoContainer px-12 py-10 h-3/5 max-h-full  bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg shadow-lg shadow-slate-950">
        ...
        <ul className="mt-10 flex flex-col flex-wrap">
          {todos.map((todo) => (
            <li
              key={todo}
              className="text-slate-50 px-2 border-b border-slate-500 pb-3 pt-3  flex items-center justify-between text-wrap hover:bg-slate-500 hover:bg-opacity-40">{todo}
              <button
      ...
      <aside className="border p-10 order-first">
        <h1
        className="text-3xl text-slate-100">To Do List</h1>
      </aside>
    </section>
  );
